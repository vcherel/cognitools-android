...
adb shell am broadcast -a androidx.profileinstaller.action.INSTALL_PROFILE com.example.myapp/androidx.profileinstaller.ProfileInstallReceiver
adb shell cmd package compile -m speed-profile -f com.example.myapp
```

The last two lines matter: `adb install` leaves the app uncompiled (`status=verify`) until the phone's overnight idle job, which costs about 25% on cold start and makes everything run on the JIT. The broadcast installs the baseline profile shipped in the APK, the compile applies it; compiling without the broadcast first compiles an empty profile. Full `-m speed` measured slower than `speed-profile`. Neither touches app data.

Regenerating the baseline profile (`./gradlew :app:generateReleaseBaselineProfile -Pandroid.injected.androidTest.leaveApksInstalledAfterRun=true`) needs the phone unlocked for about 10 minutes, and MIUI sleeps it mid run: send `adb shell input keyevent KEYCODE_SHIFT_LEFT` every 10 s in a background loop (a harmless keypress resets the screen timeout; KEYCODE_WAKEUP does not, the phone locked with it). The flag keeps the app installed, so the data survives, but the journey types into a new note on the real data; back the data dir up first (install the debug APK over the release, `adb exec-out run-as com.example.myapp tar cf - .`).
...
- **Singletons**: `MyApplication` holds FlashcardRepository, DeezerRepository, PodcastRepository, NewsRepository and MailRepository. Screens reach them as `context.flashcardRepository` / `context.deezerRepository` / `context.podcastRepository` / `context.newsRepository` / `context.mailRepository` (extensions declared in `MyApplication.kt`), never by casting. Anything long lived hangs off there, not off an object/DI graph.
...
- **HTTP**: `Http.kt`'s `httpGet` serves Weather, Wikipedia, the podcast feeds/directory and the news feeds/pages. Deezer has its own client in `deezer/DeezerApi.kt`, mail goes over IMAP through JavaMail in `mail/MailRepository.kt`, podcast episode audio is fetched by hand in `PodcastDownloads.openAudio` (tracking prefixes need manual redirect following), Gallery does no networking.
